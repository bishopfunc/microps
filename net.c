...
int 
net_device_output(struct net_device *dev, uint16_t type, const u_int8_t *data, size_t len, const void *dst)
{
//dst?
    if (!NET_DEVICE_IS_UP(dev)) {
        errorf("not opened, dev=%s", dev->name);
...
net_protocol_register(uint16_t type, void (*handler)(const uint8_t *data, size_t len, struct net_device *dev))
{
    ...
    protocols = proto; //プロトコルリストの先頭に追加
    infof("registered, type=0x%04x", type);
    return 0;
}

int
net_input_handler( uint16_t type, const u_int8_t *data, size_t len, struct net_device *dev)
{
...
net_run(void)
{
    ...
    if (intr_run() == -1) {
        errorf("intr_run() failure"); //割り込み機構の起動
        return -1;
    }
...
{
    if (intr_init() == -1) {
        errorf("intr_init() failure"); //割り込み機構の初期化
        return -1;
    }
