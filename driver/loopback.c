#include <stdio.h>
#include <stddef.h>
#include <stdint.h>
#include <string.h>

#include "platform.h"

#include "util.h"
#include "net.h"

#define LOOPBACK_MTU UINT16_MAX /* maximum size of IP datagram */
#define LOOPBACK_QUEUE_LIMIT 16
#define LOOPBACK_IRQ (INTR_IRQ_BASE+1)
//また+1してる？

#define PRIV(x) ((struct loopback *)x->priv)

//ループバックデバイスのドライバ内で使用するプライベートなデータを格納るるための構造体
struct loopback {
    int irq;
    mutex_t mutex;
    struct queue_head queue;
};

struct loopback_queue_entry {
    uint16_t type;
    size_t len;
    uint8_t data[]; /* flexible array member */
    //構造体の最後なら
};

static int
loopback_transmit(struct net_device *dev, uint16_t type, const uint8_t *data, size_t len, const void *dst)
{
    struct loopback_queue_entry *entry;
    unsigned int num;

    mutex_lock(&PRIV(dev)->mutex); //キューへのアクセスをmutexで保護する
    //キューの上限を超えていたら
    if (PRIV(dev)->queue.num >= LOOPBACK_QUEUE_LIMIT) {
        mutex_unlock(&PRIV(dev)->mutex); //unlock
        errorf("queue is full");
        return -1;
    }
    entry = memory_alloc(sizeof(*entry) + len);
    if (!entry) {
        mutex_unlock(&PRIV(dev)->mutex); //unlock
        errorf("memory_alloc() failure");
        return -1;
    }

    entry->type = type;
    entry->len = len;
    memcpy(entry->data, data, len); //指定バイト数分のメモリをコピーする
    queue_push(&PRIV(dev)->queue, entry);//キューへの格納
    num = PRIV(dev)->queue.num;
    mutex_unlock(&PRIV(dev)->mutex);//このタイミングでunlock?
    debugf("queue pushed (num:%u), dev=%s, type=0x%04x, len=%zd", num, dev->name, type, len);
    debugdump(data, len);
    intr_raise_irq(PRIV(dev)->irq);//割り込みを発生させる
    return 0;
}

static int
loopback_isr(unsigned int irq, void *id)
{
    struct net_device *dev;
    struct loopback_queue_entry *entry;

    dev = (struct net_device *)id; //こんなあり？
    mutex_lock(&PRIV(dev)->mutex);

    while(1) {
        entry = queue_pop(&PRIV(dev)->queue);
        if(!entry){
            break;
        }
        debugf("queue popped (num:%u), dev=%s, type=0x%04x, len=%zd", PRIV(dev)->queue.num, dev->name, entry->type, entry->len);
        debugdump(entry->data, entry->len);
        net_input_handler(entry->type, entry->data, entry->len, dev);//受信データを渡す
        memory_free(entry);
    }
    mutex_unlock(&PRIV(dev)->mutex);
    return 0;
}

static struct net_device_ops loopback_ops = {
    .transmit = loopback_transmit,
    //この書き方やっぱわからん
};

struct net_device *
loopback_init(void)
{
    struct net_device *dev;
    struct loopback *lo;
    //Exercise 3-1: デバイスの生成とパラメータの設定
    dev = net_device_alloc();
    if (!dev) {
        errorf("net_device_alloc() failure");
        return NULL;
    }
    lo = memory_alloc(sizeof(*lo));
    if (!lo){
        errorf("memory_alloc() failure");
        return NULL;
    }
    lo->irq = LOOPBACK_IRQ;
    mutex_init(&lo->mutex); //pthreadに必要な
    queue_init(&lo->queue);
    dev->priv = lo; //プライベートなデータとは？
    //Exercise 3-2: デバイスの登録と割り込みハンドラの設定

    dev->type = NET_DEVICE_TYPE_DUMMY;
    dev->mtu = LOOPBACK_MTU;
    dev->hlen = 0; /* non header */
    dev->alen = 0; /* non address */
    dev->ops = &loopback_ops;
    dev->flags = NET_DEVICE_FLAG_LOOPBACK;
    if (net_device_register(dev) == -1) {
        errorf("net_device_register() failure");
        return NULL;
    }
    intr_request_irq(LOOPBACK_IRQ, loopback_isr, INTR_IRQ_SHARED, dev->name, dev); //割り込みハンドラとして loopback_isr を登録する
    debugf("initialized, dev=%s", dev->name);
    return dev;
}